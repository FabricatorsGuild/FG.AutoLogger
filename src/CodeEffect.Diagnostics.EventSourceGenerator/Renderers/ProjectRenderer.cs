        ...
        private void AddGeneratedOutputsToProject(Project model, bool saveChanges = true)
        {
            ...
            foreach (var include in includes)
            {
                ...
                try
                {
                    ...
                    var matchingItems = projectFiles.Where(item => item.Name.Matches($"{includeName}", StringComparison.InvariantCultureIgnoreCase));
                    ...
                    if (!alreadyIncluded)
                    {
                        ...
                        {
                            var hash = StringMatchExtensions.ToHex(include.Output.ToMD5());
                            project.AddFileToProject(includeName, "None", new Dictionary<string, string>
                            {
                        ...
                        else if (include.ItemType == ProjectItemType.ProjectSummary)
                        {
                            project.AddFileToProject(includeName, "Content", new Dictionary<string, string>
                            {
                                { "AutoGen", "true"},
                                {"AutoGenerated", metadata},
                                { "DependentUpon",
                                    include.DependentUpon.Include.RemoveCommonPrefix(includeName, System.IO.Path.DirectorySeparatorChar)},
                            });
                            addedItems.Add(includeName);
                        }
                        else
                        {
                            project.AddFileToProject(includeName, "Compile", new Dictionary<string, string>
                            {
                                {"AutoGen", "true"},
                                {"AutoGenerated", metadata},
                                { "DependentUpon",
            ...
            if (autoGeneratedDefaultEventSource != null)
            {
                ...
                else
                {
                    ...
                    var hashCheck = StringMatchExtensions.ToHex(fileContent.ToMD5());
