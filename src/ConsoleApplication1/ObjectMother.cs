...
namespace FG.Diagnostics.AutoLogger.Samples.ConsoleApplication1
{
    ...
    {
        private string[] _adjective = new string[]
        {
            "admiring",
            "adoring",
            "affectionate",
        ...
        private static readonly ObjectMother _current;
        private Random _random;
        private MD5 _md5;

        private IDictionary<Guid, string> _used;

        public static ObjectMother Current => _current;

        static ObjectMother()
        {
            var random = new Random(Environment.TickCount);
            _current = new ObjectMother(random);
        }

        private ObjectMother(Random random)
        {
            _used = new ConcurrentDictionary<Guid, string>();
            _random = random;
            _md5 = MD5.Create();
        }
        ...
        {
            var name = $"{_adjective[_random.Next(0, _adjective.Length)]}-{_adjective[_random.Next(0, _adjective.Length)]}-{_names[_random.Next(0, _names.Length)]}";
            var hash = Hash(name);
            while (_used.ContainsKey(hash))
            {
                name = $"{_adjective[_random.Next(0, _adjective.Length)]}-{_adjective[_random.Next(0, _adjective.Length)]}-{_names[_random.Next(0, _names.Length)]}";
                hash = Hash(name);
            }
            _used.Add(hash, name);
        ...
        private Guid Hash(string value)
        {
            var hash = _md5.ComputeHash(Encoding.Default.GetBytes(value));
