...
namespace PersonActor
{
	...
	internal class PersonActor : Actor, IPersonActor
	{
		...
		protected override Task OnActivateAsync()
		{
			...
			return this.StateManager.AddOrUpdateStateAsync("state", new Person(){Name = this.GetActorId().GetStringId(), Title = "Unknown"}, (key, value) => value, CancellationToken.None);			
		}
		
		public async Task<Person> GetPersonAsync(CancellationToken cancellationToken)
		{
			var state = await this.StateManager.TryGetStateAsync<Person>("state", cancellationToken);
			return state.Value;
		}
